...
class ResumeAITester:
    ...
    def test_upload_resume(self):
        ...
        if not pdf_path:
            self.log_test("Upload Resume", False, "Could not create test PDF")
            return False

        try:
            with open(pdf_path, 'rb') as f:
                files = {'file': ('test_resume.pdf', f, 'application/pdf')}
                response = requests.post(f"{self.base_url}/upload-resume", files=files, timeout=30)
            ...
            if success:
                ...
                details = f"Session ID: {self.session_id}, Sections: {data.get('sections_found', [])}"
            else:
                details = f"Status: {response.status_code}, Response: {response.text}"
            
            self.log_test("Upload Resume", success, details)
            return success
        except Exception as e:
            self.log_test("Upload Resume", False, str(e))
            return False

    def test_analyze_resume(self):
        """Test resume analysis endpoint"""
        if not self.session_id:
            self.log_test("Analyze Resume", False, "No session ID from upload")
            return False
        ...
        try:
            ...
            response = requests.post(f"{self.base_url}/analyze", json=payload, timeout=60)
            
            success = response.status_code == 200
            if success:
                data = response.json()
                score = data.get('score_before', 0)
                suggestions_count = len(data.get('suggestions', []))
                details = f"ATS Score: {score}, Suggestions: {suggestions_count}"
            else:
                details = f"Status: {response.status_code}, Response: {response.text}"
    ...
    def test_get_session(self):
        ...
        try:
            ...
            if success:
                ...
                suggestions_count = len(data.get('suggestions', []))
                details = f"Status: {status}, Suggestions: {suggestions_count}"
            else:
                details = f"Status: {response.status_code}, Response: {response.text}"
    ...
    def test_approve_changes(self):
        ...
        if not self.session_id:
            self.log_test("Approve Changes", False, "No session ID")
            return False
        ...
        try:
            ...
            if session_response.status_code != 200:
                self.log_test("Approve Changes", False, "Could not get session data")
                return False
            ...
            if not suggestions:
                self.log_test("Approve Changes", False, "No suggestions to approve")
                return False
            
            # Approve first suggestion, reject others
            approved_ids = [suggestions[0]['id']] if suggestions else []
            rejected_ids = [s['id'] for s in suggestions[1:]] if len(suggestions) > 1 else []
            ...
            response = requests.post(f"{self.base_url}/approve", json=payload, timeout=60)
            ...
            self.log_test("Approve Changes", success, details)
            return success
        except Exception as e:
            self.log_test("Approve Changes", False, str(e))
            return False
